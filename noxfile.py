...
shutil.rmtree('src/llmstudio_py_buildfun.egg-info', ignore_errors=True);
for f in glob.glob('src/llmstudio_py_buildfun/*.so'): os.remove(f);
for f in glob.glob('src/llmstudio_py_buildfun/*.c'): os.remove(f);
"""

nox.options.default_venv_backend = "uv"

package = "llmstudio_py_buildfun"
...
def tests_compiled(session: nox.Session) -> None:
    ...
    session.install("-e", ".", env={"LLMSTUDIO_PY_BUILDFUN_COMPILE_MYPYC": "1"})
